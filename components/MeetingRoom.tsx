      ...
      <div className="fixed bottom-0 flex w-full items-center justify-center gap-5 flex-wrap">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="text-white border-dark-1 bg-dark-1">
            {["Grid", "Speaker-Left", "Speaker-Right"].map((currLayout) => (
              <DropdownMenuItem
                key={currLayout}
                className={cn(
                  "hover:bg-[#4c535b] cursor-pointer",
                  layout === currLayout.toLowerCase() ? "bg-[#4c535b]" : ""
                )}
                onClick={() =>
                  setLayout(currLayout.toLowerCase() as CallLayoutType)
                }
              >
                {currLayout}
              </DropdownMenuItem>
